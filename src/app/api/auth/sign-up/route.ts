...
type PublicSignUpRequest = components["schemas"]["PublicSignUpRequest"];

export async function POST(request: Request) {
  const body = await readJsonBody<PublicSignUpRequest>(request);
  const client = createIwmAuthClient(request);
  const result = await client.POST("/api/auth/sign-up", { body: asJsonBody(body) });
